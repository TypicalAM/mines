...
func TextBoxEx(font rl.Font, bounds rl.Rectangle, text string, fakeText string, hovered bool, size float32, maxChars int) string {
	...
	if text == "" && state == rg.Normal {
		textSize = rl.MeasureTextEx(font, fakeText, size, 0)
		rl.DrawTextEx(font, fakeText, rl.Vector2{X: float32(bound.X+bound.Width/2) - textSize.X/2, Y: float32(bound.Y+bound.Height/2) - textSize.Y/2}, size, 0, rg.GetStyleColor(rg.TextboxTextColor))
	} else {
		textSize = rl.MeasureTextEx(font, text, size, 0)
		rl.DrawTextEx(font, text, rl.Vector2{X: float32(bound.X+bound.Width/2) - textSize.X/2, Y: float32(bound.Y+bound.Height/2) - textSize.Y/2}, size, 0, rg.GetStyleColor(rg.TextboxTextColor))
	}
